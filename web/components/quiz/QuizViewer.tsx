...
function isInsideMath(source: string, offset: number): boolean {
...
function replaceOutsideMath(
  ...
  return source.replace(pattern, (match, ...args) => {
    const offset = args[args.length - 2] as number;
    if (isInsideMath(source, offset)) return match;
    return replacer(match);
  });
}

function normalizeLatexSymbols(source: string) {
  let next = source
    .replaceAll("∫∫", "\\iint")
    .replaceAll("∫", "\\int")
    .replaceAll("Σ", "\\sum")
    ...
    .replaceAll("×", "\\times")
    .replaceAll("÷", "\\div");
  next = next.replace(/√\(([^()]+)\)/g, "\\sqrt{$1}");
  next = next.replace(/√([A-Za-z0-9_]+(?:\^[A-Za-z0-9{}]+)?)/g, "\\sqrt{$1}");
  return next;
}

function polishLatexExpression(source: string) {
  let next = normalizeLatexSymbols(source)
    .replace(/\$/g, "")
    .replace(/\^\{?∞\}?/g, "^{\\infty}")
    .replace(/_\{\s*([^}]+?)\s*\}/g, (_, value: string) => {
      return `_{${value.replace(/\s+/g, "")}}`;
    })
    .replace(/\^\{\s*([^}]+?)\s*\}/g, (_, value: string) => {
      return `^{${value.replace(/\s+/g, "")}}`;
    });

  const quotientMatch = next.match(
    /^(\\sum\s*(?:_\{[^}]+\})?\s*(?:\^\{[^}]+\})?)\s+(.+?)\s*\/\s*(\\sqrt\{[^}]+\})$/,
  );
  if (quotientMatch) {
    const [, operator, numerator, denominator] = quotientMatch;
    next = `${operator} \\frac{${numerator.trim()}}{${denominator}}`;
  }

  return next.replace(/\s+/g, " ").trim();
}

function displayMath(markup: string) {
  return `\n\n$$\n${polishLatexExpression(markup)}\n$$\n\n`;
}

function inlineMath(markup: string) {
  return `$${polishLatexExpression(markup)}$`;
}

function normalizeQuizMathMarkdown(source: string) {
  if (!source.trim()) return source;
  if (/(^|[^\\])\$|\\\(|\\\[/.test(source)) return source;

  let next = source;
  next = replaceOutsideMath(
    next,
    /∫∫?\s*(?:_[A-Za-z0-9{}]+)?\s*[^，。；,;!?！？\n]+/g,
    (match) => displayMath(match.trim()),
  );
  next = replaceOutsideMath(
    next,
    /(?:Σ|∑|\\sum)\s*(?:_\{?[^}\s]+\}?|\^\{?[^}\s]+\}?|\s|[()+\-*/^{}=A-Za-z0-9_\\√∞])+?(?=\s*(?:是否|收敛|发散|，|。|；|,|;|!|\?|！|？|\n|$))/g,
    (match) => displayMath(match.trim()),
  );
  next = replaceOutsideMath(
    next,
    /(?:√|\\sqrt)\s*(?:\([^，。；,;!?！？\n]+\)|\{[^}]+\}|[A-Za-z0-9_]+(?:\^[A-Za-z0-9{}]+)?)/g,
    (match) => inlineMath(match.trim()),
  );
  next = replaceOutsideMath(
    next,
    /[A-Za-z](?:[A-Za-z0-9_]*)(?:\^[A-Za-z0-9{}]+)?(?:\s*[+\-*/]\s*[A-Za-z0-9_]+(?:\^[A-Za-z0-9{}]+)?)*\s*=\s*-?\d+(?:\.\d+)?/g,
    (match) => inlineMath(match.trim()),
  );
  return next;
}
...
function QuizMarkdown({
  ...
  return (
    <MarkdownRenderer
      content={normalizeQuizMathMarkdown(content)}
      variant="prose"
      enableMath
      className={`quiz-math ${className}`.trim()}
    />
  );
}
                  ...
                  <MarkdownRenderer
                    content={
                      ...
                      !q.correct_answer.trimStart().startsWith("```")
                        ? `\`\`\`python\n${q.correct_answer}\n\`\`\``
                        : normalizeQuizMathMarkdown(q.correct_answer)
                    }
                    variant="prose"
                    enableMath
                  />
            ...
            {q.explanation && (
              <div>
                ...
                <div className="text-[13px] leading-relaxed text-[var(--muted-foreground)]">
                  <QuizMarkdown content={q.explanation} />
                </div>
              </div>
            )}
          </div>
